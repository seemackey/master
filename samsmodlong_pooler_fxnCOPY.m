...
stdidx=9; % comparison stim for rate
bmf=28; %bmf for rate
...
for perm = 1:1:perms                            % permutation loop
    for unit = 1:1:size(data)                  % neuron loop
        ...
        % bin the spikes 
        [binnedspc,stimspc,sortedspikes] = mfSort(data{unit,1},dur,int);

        % loop through the MTF function to get responses at each mf
        [levelspikes,onefreq] = MTF(binnedspc,dur,int,stimspc,sortedspikes);
