function [All] = MonkeyTankPlotterOfflineFxn(path)
...
%     [freq, levl, nlvl, Responses, TimeOfResponse] = EndTrialData(freq, levl, nlvl, Responses, TimeOfResponse); % Deletes trials after data ended
    ...
    LevelsUsed = dBtoSPL(ToneLevel, alpha); %converts tone levels from voltages to dbSPL, check which room!
...
%     plot(ProbabilityCorrect(2,2:end),ProbabilityCorrect(1,2:end),'Marker','o')
 ...
 %%   %%%%%%%%%%%%%%%%%Creates Weibull Fit%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    createFitWB(HR,PC,ProbabilityCorrect(2,2),48)
    ...
    fprintf(fid1,'%4.4f\t',Duration);
    fprintf(fid1,'%4.4f\n',Frequency);
    fclose(fid1);
    % saves the psych function figure to current directory
    % no idea why this no longer works
    %saveas(figure(3),[pwd '\DurPsychFxns\' Monkey Date Num '.fig']);
    %saveas(figure(3),[pwd '/plots/' Frequency NLevel '.fig']);
